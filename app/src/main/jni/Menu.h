
bool titleValid = false, headingValid = false, iconValid = false;

void *antiLeech(void *) {
    sleep(15);
    if (!titleValid || !headingValid || !iconValid) {
        //Bad function to make it crash
        //   int *p = 0;
        // *p = 0;
    }
    return NULL;
}

const char *icon = OBFUSCATE(
        "");


jstring IconWebViewData(JNIEnv *env, jobject thiz) {
    iconValid = true;
    //WebView support GIF animation. Upload your image or GIF on imgur.com or other sites

    // From internet (Requires android.permission.INTERNET)
    // return env->NewStringUTF(OBFUSCATE_KEY("https://i.imgur.com/SujJ85j.gif", 'd'));

    // From assets folder: (Requires android.permission.INTERNET)
    // return env->NewStringUTF(OBFUSCATE_KEY("file:///android_asset/example.gif", 'r');

    // Base64 html:
    // return env->NewStringUTF("data:image/png;base64, <encoded base64 here>");

    // To disable it, return NULL. It will use normal image above:
    // return NULL

    //return env->NewStringUTF(OBFUSCATE_KEY("https://i.imgur.com/SujJ85j.gif", 'u'));
    return NULL;
}

extern "C" {
JNIEXPORT jstring JNICALL
Java_fang_cybertron_modmenu_FloatingModMenuService_getInfo(JNIEnv *env, jobject thiz, jstring str) {
    const char *info = env->GetStringUTFChars(str, 0);

    if (info == std::string("TEXTHIDDEN"))
        return env->NewStringUTF(OBFUSCATE("Icon hidden. Remember the hidden icon position"));
    else if (info == std::string("TEXTCLOSED"))
        return env->NewStringUTF(OBFUSCATE("Menu service killed"));
    else if (info == std::string("ICON"))
        return env->NewStringUTF(icon);
    else if (info == std::string("ICONWEBVIEW"))
        return IconWebViewData(env, thiz);
    else if (info == std::string("TITLE"))
        return env->NewStringUTF(OBFUSCATE("<b>CYBERTRON</b>"));
    else if (info == std::string("HEADING"))
        return env->NewStringUTF(OBFUSCATE("<b>Mobile Legends: Bang Bang</b>"));
    else if (info == std::string("TEXTSAVEPREF"))
        return env->NewStringUTF(OBFUSCATE(
                                         "Save preferences was been enabled. Waiting for game lib to be loaded...\n\nForce load menu may not apply mods instantly. You would need to reactivate them again"));
    else if (info == std::string("TEXTFORCELOAD"))
        return env->NewStringUTF(OBFUSCATE("Force load menu"));
    else if (info == std::string("TEXTYES"))
        return env->NewStringUTF(OBFUSCATE("YES"));
    else if (info == std::string("TEXTNO"))
        return env->NewStringUTF(OBFUSCATE("NO"));
    else if (info == std::string("TEXTHIDE"))
        return env->NewStringUTF(OBFUSCATE("KILL"));
    else if (info == std::string("TEXTCLOSE"))
        return env->NewStringUTF(OBFUSCATE("HIDE"));

    // *** Themes *** //
    else if (info == std::string("HIDECLOSE_CL"))
        return env->NewStringUTF(OBFUSCATE("#EE2F353C"));
    else if (info == std::string("TEXTHIDECLOSE_CL"))
        return env->NewStringUTF(OBFUSCATE("WHITE"));
    else if (info == std::string("MAIN_CL"))
        return env->NewStringUTF(OBFUSCATE("#80000000"));
    else if (info == std::string("TXT_CL"))
        return env->NewStringUTF(OBFUSCATE("#FF0000"));
    else if (info == std::string("TXT2_CL"))
        return env->NewStringUTF(OBFUSCATE("WHITE"));
	else if (info == std::string("TXT3_CL"))
        return env->NewStringUTF(OBFUSCATE("BLACK"));
    else if (info == std::string("TXTVIEV_CL"))
        return env->NewStringUTF(OBFUSCATE("#00FFFF"));
    else if (info == std::string("HEADING"))
        return env->NewStringUTF(OBFUSCATE("#FFFFFF"));
    else if (info == std::string("BTN_CL"))
        return env->NewStringUTF(OBFUSCATE("#15CD72"));
    else if (info == std::string("SWITCHON_CL"))
        return env->NewStringUTF(OBFUSCATE("#009688"));
    else if (info == std::string("SWITCHON2_CL"))
        return env->NewStringUTF(OBFUSCATE("WHITE"));
    else if (info == std::string("SWITCHONBG_CL"))
        return env->NewStringUTF(OBFUSCATE("#4CAF50"));
    else if (info == std::string("SWITCHOFF_CL"))
        return env->NewStringUTF(OBFUSCATE("WHITE"));
    else if (info == std::string("SWITCHOFF2_CL"))
        return env->NewStringUTF(OBFUSCATE("BLACK"));
    else if (info == std::string("SWITCHOFFBG_CL"))
        return env->NewStringUTF(OBFUSCATE("#800080"));
    else if (info == std::string("NUMTXT_CL"))
        return env->NewStringUTF(OBFUSCATE("#ffe500"));
    else if (info == std::string("SEEKBAR_CL"))
        return env->NewStringUTF(OBFUSCATE("#2196F3"));

    return NULL;
}

JNIEXPORT jstring JNICALL
Java_fang_cybertron_modmenu_FloatingModMenuService_getTheme(JNIEnv *env, jobject thiz, jstring str) {
    const char *info = env->GetStringUTFChars(str, 0);

    return NULL;
}

//Canvas implementation
JNIEXPORT jboolean JNICALL
Java_fang_cybertron_modmenu_FloatingModMenuService_isReady(JNIEnv *env, jclass clazz) {
    // Make one yourself ;)
}
JNIEXPORT jboolean JNICALL
Java_fang_cybertron_modmenu_FloatingModMenuService_needEsp(JNIEnv *env, jclass clazz) {
    // Make one yourself ;)
}
JNIEXPORT void JNICALL
Java_fang_cybertron_modmenu_FloatingModMenuService_DrawOn(JNIEnv *env, jclass clazz, jobject e_spview,
                                                  jobject canvas, jint i, jint i2) {
    // Make one yourself ;)
}
}
